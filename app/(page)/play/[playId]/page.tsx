...
export default function Play({
  ...
  const [vidCard, setVidCard] = useState<any[]>([]);
              ...
              <div className="flex flex-col lg:flex-row justify-start">
                {video.map((item, index) => {
                  const thumbnailUrl = `${apiUrl}/uploads/thumbnails/${item.thumbnail
                    .split("\\")
                    .pop()}`;
                  const videoUrl = `${apiUrl}/uploads/videos/${item.videos
                    .split("\\")
                    .pop()}`;
                  const profileUrl = `${apiUrl}/uploads/profile/${item.videos
                    .split("\\")
                    .pop()}`;

                  return (
                    <div key={index} className="w-full p-2 sm:p-4">
